<?php
  ...
  function print_list($list){
    $dbQuery = mysqli_connect(HOST, USER, PASS, DBNAME) or die("Error in the query of functions print list -> ".mysqli_connect_error());
  ...
  function delete_income_expenses($list, $id){
    $dbQuery = mysqli_connect(HOST, USER, PASS, DBNAME) or die("Error in the query of functions information card -> ".mysqli_connect_error());
    ...
    if($list=='income'){
      echo '
        ...
        <li class="list-group-item"><button type="button" class="btn btn-outline-danger btn-sm" id="'.$information_card['id'].'" style="float:left;">DELETE</button><small style="float: right;color:gray;">'.$information_card['by_username'].'</small></li>';
      }else{
      echo '
